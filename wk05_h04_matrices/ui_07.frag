...
//     st -= translate*0.2;
    ...
    float field1 = smoothstep(1.5,2.8,d1);
    
    color += vec3(fract(field1))*vec3(0.99*cos(u_time),.3*sin(u_time),.27*cos(u_time)); //99% 30% 27%
    float field2 = smoothstep(0.1,0.15,d2);
    
//     color += (1.-vec3((field2)))*vec3(.99,.62,.16); //center circle 99% 62% 16%
    
    st = scale( vec2(sin(u_time/10.)+1.0) ) * 2.*st;
    st = rotate2d( sin(2.*u_time/40.)*2.*PI ) * st;

    st+=1.;
    st/=2.;
	
    // Add the shape on the foreground
    color += (5.*vec3(crossy(st,1.8))*vec3(.99,.62,.16))*(5.*vec3(crossx(st,0.01))*vec3(.99,.62,.16)); // y-axis 
    
    color += (5.*vec3(crossx(st,1.8))*vec3(.99,.62,.16))*(5.*vec3(crossy(st,0.01))*vec3(.99,.62,.16)); //x-axis
    ...
    st += translate*.8; 
    st = scale( vec2(sin(u_time/2.)+1.0) ) * st;
    color += (5.*vec3(crossx(st,0.05)*vec3(.99,.62,.16))*(5.*vec3(crossy(st,0.05))*vec3(.99,.62,.16))); // square 
...
//     st = scale( vec2(sin(u_time)+1.0) ) * st;
  ...
  color += (1.-vec3(smoothstep(.4,.41,d)))*vec3(.99,.62,.16); // triangle 99% 62% 16%
    ...
    color += (5.*vec3(crossx(st,0.02)*vec3(.07,.5,.49))*(5.*vec3(crossy(st,0.02))*vec3(.07,.5,.49))); // dot  7% 50% 49%
...
//     st = scale( vec2(sin(u_time/10.)+1.0) ) * st;
    d = 8.*field(30.,0.5,0.5,st); // Number of sides, x position, y position, st

  color += (1.-vec3(smoothstep(.4,.41,d)))*vec3(.07,.5,.49); // circle
...
//     st = st *2.-1.;
    ...
    st = rotate2d( sin(2.*u_time/4.)*2.*PI ) * st;
//     st = scale( vec2(sin(u_time/5.)+1.0) ) * st;
    
    d4 = 8.*field(30.,0.5,0.5,st); // Number of sides, x position, y position, st

  color += (1.-vec3(smoothstep(.6,.61,d4)))*vec3(.07,.5,.49); // circle
    gl_FragColor = vec4(1.-(sin(60.*u_time)*color),1.0);
}
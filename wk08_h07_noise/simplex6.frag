...
void main() {
	...
	vec3 color = vec3(snoise(vec2(abs(sin(u_time/20.)))));
    ...
    vec2 pos = vec2(st*5.)-snoise(vec2(noise(st*10.*(.5+abs(sin(u_time/2.))))));
  ...
  // Shaping function that modulate the distance
    d1 = field(6.,.5,.5,st)/4.1; // Number of sides, x position, y position, st
	
    d2 = 3.*field(6.,0.5,0.5,st)/9.;    
    ...
    gl_FragColor = vec4(1.-vec3(color.r+n, color.g-n, color.b/3.*n), 1.0);
}
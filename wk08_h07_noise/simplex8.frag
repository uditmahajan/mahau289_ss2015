...
void main() {
	...
	vec3 color = vec3(snoise(vec2(abs(sin(u_time/2.)))));
  ...
  // Shaping function that modulate the distance
    d1 = field(3.,.5,.5,st)/4.1; // Number of sides, x position, y position, st
	
    d2 = 3.*field(3.,0.5,0.5,st)/9.;    
    ...
    gl_FragColor = vec4(vec3(color.r*n, 0., color.b/10.), 1.0);
}
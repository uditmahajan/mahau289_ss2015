...
//   color = vec3(smoothstep(.5,.52,max(d-abs(sin(u_time)),d2-abs(cos(u_time)))));
    ...
    color = vec3(smoothstep(.5,.52,min(min(d*1.6,d2*1.1), min(d3, d4)))-smoothstep(.5,.52,min(min(d3*1.6,d4*1.1), min(d*1.2, d2*.93)))); 
    color -= vec3(smoothstep(0.3,.32,d3*d2*4.)-smoothstep(0.3,.32,d3*d4*5.));
  // color = vec3(d);

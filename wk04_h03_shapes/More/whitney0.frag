...
//   float r = TWO_PI/float(N);
  ...
  d2 = field(2.,0.1,abs(sin(u_time)),st);
  d3 = field(40.,0.5,abs(cos(u_time/2.)),st);
  d4 = field(3.,abs(cos(u_time)),0.4,st);

//   color = vec3(smoothstep(.5,.52,max(d-abs(sin(u_time)),d2-abs(cos(u_time)))));
    
  
    float field1 = smoothstep(.3,.32,d2)-smoothstep(.3,.32,d);
    
    color = vec3(2.*fract(field1*30.)); 
    
    float field2 = smoothstep(0.9,.92,d4)-smoothstep(0.5,.52,d3);

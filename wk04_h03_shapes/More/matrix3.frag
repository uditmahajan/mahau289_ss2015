...
//   float r = TWO_PI/float(N);
  ...
  d = field(4.,abs(sin(u_time/2.)),0.5,st); // Number of sides, x position, y position, st
  
  d2 = field(4.,0.1,abs(sin(u_time)),st);
  d3 = field(4.,0.5,abs(cos(u_time/2.)),st);
  d4 = field(4.,abs(cos(u_time)),0.4,st);
